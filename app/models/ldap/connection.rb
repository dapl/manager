...
module LDAP
  class Connection
    ...
    def user_update(login, attrs)
      ops = []
      user = get_user(login)
      update = {}
      update[:gecos] = attrs[:name] if attrs[:name] != user.name
      update[:mail] = attrs[:email] if attrs[:email] != user.email
      update[:employeetype] = attrs[:employeetype] if attrs[:employeetype] != user.employeetype
      update[:loginshell] = attrs[:shell] if attrs[:shell] != user.shell
      update.delete_if { |k, v| v.nil? || v.blank? }
      update.each do |key, value|
        ops << [:replace, key.to_sym, value]
      end
      puts ops.inspect
